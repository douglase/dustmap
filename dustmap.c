...
void cos_doubles(double * in_array, double * out_array, int size){
  /*
  test this with the following python call ( a cosine figure should appear):
  import numpy as np
   ...: import pylab
   ...: import dustmap
   ...:
...
void dustmap(
  double * hist,// = (double *) argv[j]; j++;     //The final image, passed by reference
  ...
  for(ifile=0;ifile<numfiles;ifile++){ //loop through all input files
    ...
    //Create the image
    dm2d(hist, histoxsize, histoysize, data, ndatapoints, fovx, fovy, pixelsize, inclination, longitude, distance, pa, lambda, nlambda, Qabs_value, Qsca_value, dustradius[ifile], Bnustar, Rstar, Tvsr_list, delta_log_r, nr, log10minr, Tsublimate, pfunc, npfunc, Qwave, Qabs, nQwave, iwa, aitoff_flag, densityhisto_flag, opticaldepth_flag, scatteredlight_flag, thermalemission_flag, extinction_flag, xshift, effrdust, distmask);
  ...
  if(nmarks > 0) {
    ...
    if(verbose_flag >= 1) printf("Marked locations.\n");
    dmmarklocs(hist, markdata, nmarks, fovx, fovy, pixelsize, inclination, longitude, distance, pa, aitoff_flag, markabs_flag, xshift);
    if(verbose_flag >= 1) printf("Marked locations.\n");
    free(markdata);
  }
...
void dustmap_memory(
  ...
  //edits by Yinzi to allow passing of coordinates directly (datatype 3 only)
  //char *inputfilelist ,//= (char *) argv[j]; j++; //Concatenated strings of input files from IDL
  ...
  /* comment out inputfile stuff -Yinzi
  //-------------------- REWORK SOME INPUT --------------------
  //Here we reinterpret the list of input files given by IDL.
  //IDL just gives us a series of concatenated strings, separated by a null terminating
  //character.  It's more elegant to deal with an array of strings, so we manipulate it a bit.
...
/*
  ...
  */
  ...
  //Yinzi edits here to take data from passed inputs

  //for(ifile=0;ifile<numfiles;ifile++){ //loop through all input files
  for(ifile=0;ifile<1;ifile++){ //loop through all input files
    ...
    for(i=0;i<nentries;i++) {
      ...
      (data)[i].y=y_coords[i];
      (data)[i].z=z_coords[i];
      (data)[i].intensity=particle_intensity[i]*scaling[ifile];
    }
